...
eval env (ForLoop (p,initial) (i,bound) body) =
  case (eval env initial, eval env bound) of
    (Left err, _) -> Left err
    (_, Left err) -> Left err
    (Right v, Right (ValInt n)) -> loop (envExtend i (ValInt 0) (envExtend p v env)) 0 n
    _ -> Left "Non-integral loop bound"
  where
    loop lEnv iValue n =
      if iValue < n then
        case eval lEnv body of
          Left err -> Left err
          Right newP -> loop (envExtend i (ValInt (iValue + 1)) (envExtend p newP lEnv)) (iValue + 1) n
      else
        eval lEnv (Var p)
--
-- Implementation for Lambda and Apply -----------------------------------------------------------------
eval env (Lambda pname body) = Right (ValFun env pname body)
eval env (Apply e1 e2) = 
  case (eval env e1, eval env e2) of
    (Left err, _) -> Left err
    (_, Left err) -> Left err
    (Right (ValFun funEnv pname body), Right arg) -> eval (envExtend pname arg funEnv) body
    _ -> Left "Apply: Left Expression not of Type ValFun"
--
-- Implementation for Try-Catch -----------------------------------------------------------------
